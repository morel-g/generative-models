...
def variance_scaling(
    ...
    def _compute_fans(shape, in_axis=1, out_axis=0):
        receptive_field_size = (
            np.prod(shape) / shape[in_axis] / shape[out_axis]
        )
...
def ddpm_conv1x1(
    in_planes, out_planes, stride=1, bias=True, init_scale=1.0, padding=0
):
...
class MSFBlock(nn.Module):
    def __init__(self, in_planes, features):
        ...
        for i in range(len(in_planes)):
            self.convs.append(
                ncsn_conv3x3(in_planes[i], features, stride=1, bias=True)
            )
...
def contract_inner(x, y):
    ...
    y_chars = list(
        string.ascii_lowercase[len(x.shape) : len(y.shape) + len(x.shape)]
    )
