    ...
    def score_fn(u, t):
        ...
        if config.mixed_score:
            if sde.is_augmented:
                ...
                var_zz = (
                    sde.var(t, 0.0 * ones, (sde.gamma / sde.m_inv) * ones)[2]
                ).type(torch.float32)
...
def get_x0_prediction(config, sde, model, train=False):
    def x0_prediction(u, t):
        ...
        if sde.is_augmented:
            coeff = torch.exp(-2.0 * beta_int * sde.g)
            g = -beta_int * coeff
            f = (2.0 * sde.g * beta_int + 1.0) * coeff
            var = sde.var(t)
            ...
            x0 = (
                v - cholesky22 * eps_prediction - cholesky21 * x / cholesky11
            ) / (g - f * cholesky21 / cholesky11)
            return x0
        else:
            coeff = torch.exp(-0.5 * beta_int)
            return (u - (1.0 - coeff**2.0) * eps_prediction) / coeff
