...
class UNet2DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        elif time_embedding_type == "no_embedding":
            in_channels += 1
            time_embed_dim = None

        if time_embedding_type != "no_embedding":
    ...
    def forward(
        ...
        if self.time_embedding_type == "no_embedding":
            t_emb = timesteps.reshape(-1, 1, 1, 1).expand(
                (sample.shape[0], 1, sample.shape[2], sample.shape[3])
            )
            sample = torch.cat((sample, t_emb), dim=1)
            t_emb, emb = None, None
        else:
