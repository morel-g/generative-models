...
def compute_imgs_outputs(
    ...
    if net.config.model_type == Case.score_model:
        ...
        sample_img(net, output_dir, name="Samples", nb_rows=nb_rows, nb_cols=nb_cols)
...
def sample_img(
    ...
    if save_gifs:
        save_images_as_grid_video(imgs, output_dir, nb_rows, nb_cols, name=name)

    # Check if images are spectograms and save audio files if needed
    if save_audio_if_needed and net.config.data_type in audio_data_type:
